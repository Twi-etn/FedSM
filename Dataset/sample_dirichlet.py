...
def clients_indices(list_label2indices: list, num_classes: int, num_clients: int, non_iid_alpha: float, seed=None):
    ...
    print("build_non_iid_by_dirichlet: ")
    ...
    print("build_non_iid_by_dirichlet ----ending : ")
...
def build_non_iid_by_dirichlet(
    ...
    for _targets in splitted_targets:
        ...
        while min_size < int(0.50 * _targets_size / _n_workers):
            ...
            #     sizes = (proportions * len(idx_class)).astype(int)
            #     sizes[-1] = len(idx_class) - np.sum(sizes[:-1])
            #     np.random.shuffle(idx_class)
            #     idx_class = np.split(idx_class, sizes)
            #     _idx_batch = [np.concatenate((_idx_batch[i], idx_class[i])) for i in range(len(idx_class))]
            #
